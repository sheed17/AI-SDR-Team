...
- Navigate LinkedIn freely for sourcing, research, people lookup, jobs, company pages, and personalization.
- Draft personalized LinkedIn outreach for the operator to send manually.
- Do not send LinkedIn messages, connection requests, comments, reactions, follows, endorsements, or other engagement actions.
- Avoid high-volume behavior that could risk the account.
...
- The hook is grounded in Signal A or Signal B
- No disqualifier blocks outreach
- `Account Tier` is `A` or a strong `B`
- `Workflow Audit Angle` is complete

Before drafting:

- Use the best named decision-maker found in the research pass.
- For A-tier accounts, use the buying committee to choose the best persona.
- Prefer a named founder/owner/operator/ops lead over a general company greeting.
- Use `Hi team` only when no named person is findable or the person match is low confidence.
- If a named person is found but email is a general inbox, the email may still address the named decision-maker only when the public company context makes that reasonable.
...
Use a calm operator-to-operator style:
...
- "10x"
- Long feature lists
- Multi-paragraph company flattery
- Claims that are not supported by evidence
- Overfamiliar references to personal LinkedIn details
...
Use this when the operator wants copy to send manually from their main LinkedIn account.

Rules:

- Draft only; do not send.
- Keep it short enough to paste into LinkedIn.
- Use visible profile context only when it improves relevance.
- Do not mention that an alt/operator account was used for research.
- Do not imply a relationship or prior conversation.
...
- Restate the same workflow angle.
- Ask if the named workflow is worth checking.
